...
**SearXNG**:
A self-hosted metasearch engine deployed as plain manifests under `apps/searxng` (not a Helm chart, see [ADR-0010](docs/adr/0010-searxng-plain-manifests-not-truecharts-chart.md)). Stateless: configured entirely via env vars (`SEARXNG_SECRET` from ESO/1Password, `SEARXNG_BASE_URL`; no instance-level rate limiter — tried it, but it unconditionally blocks non-browser clients like Hermes regardless of request rate, so it protects against traffic that doesn't exist on a tailnet-only instance), so no PVC — but pinned to the home node (toleration + `topology.kubernetes.io/zone: home` nodeSelector, ADR-0002's scheduling pattern) for its RAM/CPU headroom. Exposed tailnet-only via the Tailscale Kubernetes operator (`type: LoadBalancer` + `loadBalancerClass: tailscale` + `tailscale.com/hostname`, the same pattern as Hermes Agent) — not the public Traefik/Cloudflare path, since a single-user search instance shouldn't be public.
...
**Authentik**:
An open-source identity provider (SSO/OIDC/SAML/LDAP) deployed as a wrapper Helm chart under `apps/authentik`, wrapping the official `goauthentik/authentik` chart (pinned `2026.8.0`) — the IdP for the tailnet. Stateful (PostgreSQL bundled subchart on the home-node `local-path` storage). Pinned to the **home node** (toleration + `topology.kubernetes.io/zone: home` nodeSelector, ADR-0002's scheduling pattern) for both the Authentik server/worker and its PostgreSQL. Exposed tailnet-only via the Tailscale Kubernetes operator (`type: LoadBalancer` + `loadBalancerClass: tailscale` + `tailscale.com/hostname: authentik`, same as SearXNG/Hermes Agent) — deliberately not on the public Traefik/Cloudflare path, since it's the tailnet's identity provider. Secrets (the `AUTHENTIK_*` config secret and the PostgreSQL password) come from ESO/1Password (`ClusterSecretStore/onepassword`), one secret per concern (`authentik-config`, `authentik-postgres-credentials`); the chart's own generated config secret is skipped.
_Avoid_: public exposure via Traefik/Cloudflare (it's the IdP for the tailnet), the chart's built-in ingress (disabled), and committing `authentik.secret_key`/`postgres` passwords to git.
